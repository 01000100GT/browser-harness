...
- **`alert()`/`confirm()` block the page thread.** Two tools: `capture_dialogs()` BEFORE the action to auto-approve and stash messages (read via `dialogs()`), or `dismiss_dialog()` AFTER to dismiss via CDP — the latter also handles `beforeunload` and works when JS is frozen. `goto()` auto-clears `beforeunload` handlers.
