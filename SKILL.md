...
- **`alert()`/`confirm()` block the page thread.** Two tools: `capture_dialogs()` BEFORE the action to auto-approve and stash messages (read via `dialogs()`) — note this injects JS that antibot can detect. Or `dismiss_dialog()` AFTER to dismiss via CDP — handles `beforeunload`, works when JS is frozen, returns `{type, message, url}` so the agent can read the dialog, and is undetectable. `goto()` auto-dismisses `beforeunload` via CDP.
